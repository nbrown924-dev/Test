...
function renderOptions(){
  const fam=currentModel.family; const groups=[...new Set(CATALOG.options.map(o=>o.group))]; optionsEl.innerHTML="";
...
function renderSpecsTable(){
  ...
  Object.keys(specs).forEach(k=>{ const tr=document.createElement('tr'); const td1=document.createElement('td'); const td2=document.createElement('td'); td1.textContent=k; td2.textContent=specs[k]; tr.appendChild(td1); tr.appendChild(td2); specsTable.appendChild(tr); });
}
...
function updateDownload(totals){
  ...
  const url=URL.createObjectURL(blob); btnDownload.href=url;
}
function updateURL(){
...
// Snapshot & Share
btnSnapshot.addEventListener('click', ()=>{ renderer.render(scene,camera); const link=document.createElement('a'); link.href=renderer.domElement.toDataURL('image/png'); link.download='skid-snapshot.png'; link.click(); });
btnShare.addEventListener('click', ()=>{ updateURL(); navigator.clipboard.writeText(location.href).then(()=>{ btnShare.textContent="Link copied!"; setTimeout(()=>btnShare.textContent="Share build",1400); }); });
btnReqQuote.addEventListener('click', ()=>{ const {total,weight,items}=computeTotals(); const body=encodeURIComponent(`I'd like a quote on this configuration:%0D%0A%0D%0AModel: ${currentModel.name}%0DOptions: ${items.join(', ')}%0DTotal: $${total}%0DWeight: ${weight} lb%0DLink: ${location.href}`); window.location.href=`mailto:sales@warriorbrushtrucks.com?subject=Skid%20Quote%20Request&body=${body}`; });

// App init
(function init(){
  CATALOG.families.forEach(f=>familySel.add(new Option(f,f)));
  familySel.addEventListener('change', ()=>{ loadModels(familySel.value); });
  function loadModels(fam){ modelSel.innerHTML=""; CATALOG.models.filter(m=>m.family===fam).forEach(m=>modelSel.add(new Option(m.name,m.id))); modelSel.dispatchEvent(new Event('change')); }
  modelSel.addEventListener('change', ()=>{
    const id=modelSel.value; const m=CATALOG.models.find(mm=>mm.id===id); currentModel=m; selected=new Set(m.defaults||[]);
    ...
    modelImg.src=m.imageUrl||""; modelImg.style.display = m.imageUrl ? 'block' : 'none';
  });
  if(!restoreFromURL()){ familySel.value=CATALOG.families[0]; loadModels(familySel.value); } else {
    familySel.value=currentModel.family; loadModels(currentModel.family); modelSel.value=currentModel.id; selected=new Set([...selected]);
    renderOptions(); buildBase(currentModel); const totals=computeTotals(); updateSummary(totals); updateDownload(totals); renderSpecsTable(); modelImg.src=currentModel.imageUrl||"";
  }
